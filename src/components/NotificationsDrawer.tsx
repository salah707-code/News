...
import { NotificationItem, AppSettings, NewsArticle } from '../types';
...
export const NotificationsDrawer: React.FC<NotificationsDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs animate-fade-in">
      <motion.div
        initial={{ x: settings.language === 'ar' ? -350 : 350, opacity: 0 }}
        animate={{ x: 0, opacity: 1 }}
        exit={{ x: settings.language === 'ar' ? -350 : 350, opacity: 0 }}
        className={`w-full max-w-sm h-full border-s shadow-2xl flex flex-col ${bgClasses.surface}`}
      >
        {/* Header */}
        <div className={`px-4 py-3.5 border-b flex items-center justify-between gap-2 ${bgClasses.header}`}>
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-xl ${theme.primaryLight}`}>
              <Bell className={`w-4 h-4 ${theme.primaryText}`} />
            </div>
            <div>
          ...
          <button
            ...
            onClick={onClose}
            className={`p-1.5 rounded-xl border ${bgClasses.surface} ${bgClasses.hover}`}
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {notifications.length > 0 && (
          <div className="px-4 py-2 border-b flex items-center justify-between text-xs bg-slate-50/50 dark:bg-neutral-900/40">
            <button
              id="mark-all-read-btn"
              onClick={onMarkAllAsRead}
              className={`flex items-center gap-1 font-semibold ${theme.primaryText} hover:underline`}
            >
            ...
            <AnimatePresence>
              {notifications.map((notif) => (
                <div key={notif.id} className="relative overflow-hidden rounded-2xl">
                  {/* Background Reveal Action on Swipe to the Right */}
                  <div className="absolute inset-0 bg-red-500/90 rounded-2xl flex items-center justify-start px-4 text-white text-xs font-bold gap-2">
                  ...
                  <motion.div
                    ...
                    className={`relative p-3.5 rounded-2xl border cursor-pointer select-none transition-shadow ${
                      !notif.read ? `${theme.primaryLight} ${theme.border}` : `${bgClasses.card} ${bgClasses.hover}`
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <div className="flex items-center gap-1.5">
                        {notif.isBreaking && (
                          <span className={`px-1.5 py-0.5 rounded ${theme.primary} text-white text-[9px] font-bold flex items-center gap-0.5 shadow-xs`}>
